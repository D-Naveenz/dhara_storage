...
/// <summary>
/// Owns the lifecycle of the Windows named-pipe dhara-sd daemon and a ready gRPC client.
/// </summary>
internal sealed class DaemonHost : IAsyncDisposable
{
    public const string DefaultPipeName = @"dhara-sd";
    ...
    private DaemonHost(Process process, GrpcChannel channel, DharaSd.DharaSdClient client, string pipeName)
    {
        _process = process;
        _channel = channel;
        Client = client;
    ...
    private static async Task WaitForReadyAsync(DharaSd.DharaSdClient client, CancellationToken cancellationToken)
    {
        ...
        throw new TimeoutException("Timed out waiting for dhara-sd daemon Ping.", last);
    }
    ...
    private static string ResolveDaemonExe()
    {
        ...
        throw new FileNotFoundException(
            "dhara-sd.exe not found. Build the dhara-sd daemon first.",
            local);
    }
