...
use dhara_storage::{
    ContentKind, DirectoryStorage, FileInfo, SharedProgressReporter, StorageChangeType,
    StorageEntry, StorageProgress, StorageWatchConfig, TransferOptions, WriteOptions, analyze_path,
    copy_file_with_options, read_file, write_file_from_reader,
};
use futures::Stream;
use tokio::sync::mpsc;
...
pub mod proto {
    tonic::include_proto!("dhara.pilot.v1");
}

use proto::dhara_pilot_server::{DharaPilot, DharaPilotServer};
use proto::*;
...
pub fn create_service(state: Arc<PilotState>) -> tonic::transport::server::Router {
    tonic::transport::Server::builder().add_service(DharaPilotServer::new(PilotService { state }))
}
...
#[tonic::async_trait]
impl DharaPilot for PilotService {
    ...
    async fn list_entries(
        ...
        let path = request.into_inner().path;
        let directory = DirectoryStorage::from_existing(&path).map_err(map_storage_error)?;
    ...
    async fn open_read_handle(
        ...
        let (handle, size) = handle_dup::open_and_duplicate(&path, parent_pid)
    ...
    async fn copy_file(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            match copy_file_with_options(&req.source, &req.destination, options) {
                Ok(destination) => {
