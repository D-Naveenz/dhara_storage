...
use crate::ui::{CommandForm, RunCompletion, RunHandle, cancel_run, start_run};
        ...
        if let Some(run) = &mut self.active_run {
            while let Ok(event) = run.output_rx.try_recv() {
                self.active_output.push(OutputLine {
                    is_error: matches!(event.stream, crate::OutputStream::Stderr),
                    text: event.line,
                });
            }
...
mod tests {
    ...
    fn poll_active_run_records_history_on_completion() {
        ...
        let context = ToolContext {
            repo_root: ".".into(),
            tool_root: ".".into(),
            run_mode: crate::command::RunMode::Interactive,
            min: false,
            trace: false,
            workers: 4,
            package_dir: None,
