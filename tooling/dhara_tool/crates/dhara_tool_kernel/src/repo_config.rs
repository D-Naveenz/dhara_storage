...
pub fn detect_config_drift(repo_root: &Path) -> Result<Vec<ConfigDriftItem>> {
    ...
    if manifest_tool_version != config.tool.version {
        drifts.push(ConfigDriftItem {
            ...
            summary: format!(
                "{TOOL_CARGO_TOML_PATH} workspace.package.version: {manifest_tool_version} -> {}",
                config.tool.version
            ),
        });
    }
...
pub fn read_tool_crate_version(repo_root: &Path) -> Result<String> {
    ...
    let version = tool_version_from_document(&document).with_context(|| {
        format!(
            "{} is missing workspace.package.version (or legacy package.version)",
            path.display()
        )
    })?;
...
mod tests {
  ...
  <ItemGroup>
    ...
    #[test]
    fn sync_tool_cargo_toml_updates_workspace_package_version() {
        let updated = sync_tool_cargo_toml(
            "[workspace.package]\nversion = \"0.8.1\"\n",
            "0.8.4",
        )
        .unwrap();
