...
pub fn sync_cargo_toml(content: &str, version: &str) -> Result<String> {
    ...
    document["workspace"]["dependencies"]["dhara_dhbin"]["version"] = value(version);
    document["workspace"]["dependencies"]["dhara_storage"]["version"] = value(version);
    document["workspace"]["dependencies"]["dhara_tool_dhara_storage"]["version"] = value(version);
...
mod tests {
    ...
    fn sample_config() -> DharaRepoConfig {
        ...
        DharaRepoConfig {
            ...
            nuget: NuGetConfig {
                ...
                icon: Some("bindings/dotnet/Dhara.Storage/icon-small.png".to_owned()),
    ...
    fn write_required_files(repo_root: &Path) {
        ...
        fs::create_dir_all(repo_root.join("bindings/dotnet/Dhara.Storage.Tests")).unwrap();
        fs::create_dir_all(repo_root.join("bindings/dotnet/Dhara.Storage.ConsumerSmoke")).unwrap();
        ...
        .unwrap();
        fs::write(
            repo_root.join("bindings/dotnet/Dhara.Storage.Tests/Dhara.Storage.Tests.csproj"),
            "<Project />",
        )
        .unwrap();
        fs::write(
        ...
        .unwrap();
        fs::write(
            repo_root.join("bindings/dotnet/Dhara.Storage/README.md"),
            "# Dhara.Storage",
        )
        .unwrap();
        fs::write(
            repo_root.join("bindings/dotnet/Dhara.Storage/icon-small.png"),
            "png",
        )
        .unwrap();
    }
    ...
    fn sync_cargo_toml_updates_workspace_version() {
        ...
        assert!(updated.contains("dhara_dhbin = { version = \"0.2.0\", path = \"dhara_dhbin\" }"));
        assert!(
            updated.contains("dhara_storage = { version = \"0.2.0\", path = \"dhara_storage\" }")
        );
        assert!(
            updated.contains(
                "dhara_tool_dhara_storage = { version = \"0.2.0\", path = \"tooling/dhara_tool_dhara_storage\" }"
            )
        );
    }
